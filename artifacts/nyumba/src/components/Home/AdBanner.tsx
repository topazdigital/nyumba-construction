...
const ads: Ad[] = [
  {
    id: 1,
    label: 'SPONSORED',
    headline: 'Bamburi Cement — Built for Kenya',
    sub: 'Premium Portland cement trusted by contractors nationwide. Distributor pricing available.',
    cta: 'Request Quote',
    ctaLink: 'https://wa.me/254700000001?text=Hi%2C%20I%20need%20cement%20pricing',
  ...
  },
  {
    id: 2,
    label: 'FEATURED',
    headline: 'Nairobi Prime Apartments — Q3 2025',
    sub: 'Studio to 3-bedroom units in Westlands. Ready 2025. Flexible payment plans from KES 5.5M.',
    cta: 'Book Viewing',
    ctaLink: '/properties',
  ...
  },
  {
    id: 3,
    label: 'ADVERTORIAL',
    headline: 'Steel Masters Kenya',
    sub: 'Structural steel, roofing sheets & fabrication. Fast delivery across Nairobi, Mombasa & Kisumu.',
    cta: 'Get Pricing',
    ctaLink: '/materials',
  ...
  },
  {
    id: 4,
    label: 'SPONSORED',
    headline: 'Architecture & Design Hub',
    sub: 'Award-winning architects available for residential, commercial & mixed-use projects across East Africa.',
    cta: 'View Profiles',
...
const AdBanner: React.FC = () => {
  ...
  const prev = () => go((current - 1 + ads.length) % ads.length);
  const next = () => go((current + 1) % ads.length);

  const ad = ads[current];

  return (
    <div className="rounded-xl overflow-hidden shadow-lg">
      <div className={`bg-gradient-to-br ${ad.bg} relative`}>
        <div className="relative h-36 overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col justify-between p-3">
            <div className="flex items-center justify-between">
              <span className={`${ad.accent} px-2 py-0.5 rounded text-[10px] font-bold tracking-wider`}>
                {ad.label}
              </span>
              <div className="flex gap-1">
                <button
                  onClick={prev}
                  className="bg-white/20 hover:bg-white/40 text-white p-1 rounded-full transition-colors"
                >
        ...
        <div className="p-3 bg-white/5 backdrop-blur-sm">
          <p className="text-gray-200 text-[11px] leading-snug mb-2.5 line-clamp-2">{ad.sub}</p>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {ad.phone && (
              <a
                ...
                className="w-full bg-transparent border border-white/30 hover:border-white/60 text-white text-[11px] py-1.5 px-3 rounded-lg transition-colors flex items-center justify-center gap-1.5"
              >
