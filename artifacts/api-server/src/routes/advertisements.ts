...
router.get("/advertisements", async (req, res) => {
  try {
    const ads = await db.select().from(advertisementsTable)
      .where(eq(advertisementsTable.active, true))
...
router.post("/advertisements/:id/click", async (req, res) => {
  try {
    await db.update(advertisementsTable)
      .set({ clicks: sql`${advertisementsTable.clicks} + 1` })
      .where(eq(advertisementsTable.id, parseInt(req.params.id)));
...
router.post("/advertisements", requireAdmin, async (req, res) => {
  try {
    const [ad] = await db.insert(advertisementsTable).values(req.body).returning();
...
router.delete("/advertisements/:id", requireAdmin, async (req, res) => {
  try {
    await db.delete(advertisementsTable)
      .where(eq(advertisementsTable.id, parseInt(req.params.id)));
    res.json({ success: true });
  } catch (err) {
    req.log.error(err);
